...
export const createRooms = async (formData) => {
  const response = await axios.post(
    "http://35.78.93.205:8080/api/room",
    formData
  );
...
export async function roomList() {
  const response = await axios.get("http://35.78.93.205:8080/api/roomList");
  // console.log(response.data);
  return response.data;
}
...
export const roomInfo = async (roomNo) => {
  const response = await axios.get(
    `http://35.78.93.205:8080/api/room/${roomNo}`
  );
  console.log(response.data);
  return response.data;
};
...
export const report = async (formData) => {
  // axios.post("http://35.78.93.205:8080/api/report", formData);

  const response = await axios.post(
    "http://35.78.93.205:8080/api/report",
    formData
  );
...
export const client_in = async (roomNo) => {
  axios.get(`http://35.78.93.205:8080/api/clientIn/${roomNo}`);
};

// 해당 room에 user --
export const client_out = async (roomNo) => {
  axios.get(`http://35.78.93.205:8080/api/clientOut/${roomNo}`);
};

// 금기어 생성하기
export const insert_taboo = async (formData) => {
  axios.post("http://35.78.93.205:8080/api/taboo", formData);
};

// 내 방 금기어 전부 가져오기 !
export const alltabooList = async (roomNo) => {
  const response = await axios.get(
    `http://35.78.93.205:8080/api/tabooList/${roomNo}`
  );
...
export const deleteTaboo = async (tabooWord) => {
  ...
  axios.get(`http://35.78.93.205:8080/api/deleteTaboo/${tabooWord}`);
};

// 5인 이하의 방 ( 방장이 없는 ) delete
export const deleteRoom = async (roomNo) => {
  axios.get(`http://35.78.93.205:8080/api/deleteRoom/${roomNo}`);
};

// // 신고 몇 번 당했는지 값 불러오기
// export const axiosReportNum = async (roomNo, userId) => {
//   const response = await axios.get(
//     `http://35.78.93.205:8080/api/reportNum?roomNo=${roomNo}&reportedId=${userId}`
//   );
